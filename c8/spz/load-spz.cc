...
void packQuaternionSmallestThree(
  uint8_t r[4], const float rotation[4], const CoordinateConverter &c) {
  ...
  for (unsigned i = 0; i < 4; ++i) {
    if (i != iLargest) {
      ...
      uint32_t mag = (uint32_t)(float((1u << 9u) - 1u) * (std::fabs(q[i]) / sqrt1_2) + 0.5f);
...
#ifdef CREATE_SPZ_VERSION_2
    // Use the first three method to pack quaternions.
    packQuaternionFirstThree(&packed.rotations[3 * i], &g.rotations[4 * i], c);
#else
    // Use the smallest three method to pack quaternions.
...
void unpackQuaternionFirstThree(
  float rotation[4], const uint8_t r[3], const CoordinateConverter &c = CoordinateConverter()) {
  Vec3f xyz = times(
    plus(
      times(
...
void unpackQuaternionSmallestThree(
  float rotation[4], const uint8_t r[4], const CoordinateConverter &c = CoordinateConverter()) {
  uint32_t comp = r[0] + (r[1] << 8) + (r[2] << 16) + (r[3] << 24);
