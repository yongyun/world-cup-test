...
const newTouch = opts => (window.Touch ? new Touch(opts) : opts)

const newTouchEvent = (type, opts) => {
  const baseOpts = {
    bubbles: true,
    cancelable: true,
  }
...
const createInteractionProvider = () => {
  ...
  const startPointer = (position, targetOverride = null) => {
    ...
    dispatch(target, newTouchEvent('touchstart', {
      view: window,
  ...
  const endPointer = (identifier, opts = {}) => {
    ...
    const endEvent = newTouchEvent('touchend', {
      view: window,
    ...
    if (!(opts as any).preventClick && !endEvent.defaultPrevented && !pointer.isMultiTouch) {
  ...
  const cancelPointer = (identifier) => {
    ...
    dispatch(target, newTouchEvent('touchcancel', {
      view: window,
      changedTouches: [getTouchFromPointer(pointer)],
      touches: activeTouches_,
      targetTouches: activeTouchesByTarget_.get(target),
  ...
  const flushMoveEvents = () => {
    ...
    movedByTarget.forEach((changedTouches, target) => {
      const endEvent = newTouchEvent('touchmove', {
        view: window,
        changedTouches,
        touches: activeTouches_,
        targetTouches: activeTouchesByTarget_.get(target),
