...
export default class UpdateManager {
   ...
   * It registers this component and all its descendants for the different types
   * of updates that were required.
   */
  ...
  static update() {
    if (Object.keys(this.requestedUpdates).length > 0) {
      const roots = this.components.filter(component => !component.parentUI)
  ...
  static traverseParsing(component) {
    ...
    // NOTE(johnny): Order is important! Setting the rebuildText flag needs to be done before
    // the other update functions.
    if (request && request.rebuildText && component.isText) {
      request.rebuildText = false
      component.rebuildText = true
    }
