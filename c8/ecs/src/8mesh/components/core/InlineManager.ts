  ...
  return class InlineManager extends Base {
    ...
    computeLines() {
      ...
      this.childrenInlines.reduce((lastInlineOffset, inlineComponent) => {
        ...
        const letterSpacing =
          inlineComponent.isText ? inlineComponent.getLetterSpacing() * fontSize : 0
        const whitespace = inlineComponent.getWhiteSpace()
        const breakOn = inlineComponent.getBreakOn()

        const currentInlineInfo = inlineComponent.inlines.reduce(
          (lastInlineInlineOffset, inline, i, inlines) => {
            ...
            // Line break
            const shouldBreak = Whitespace.shouldBreak(
              inlines, i, lastInlineInlineOffset, whitespace, letterSpacing, breakOn, innerWidth
            )
            if (shouldBreak) {
              lines.push([inline])
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        line.lineHeight = line.reduce(
          (lineHeight, inline) => Math.max(lineHeight, inline.height), 0
        )
