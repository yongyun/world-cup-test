...
template <>
inline void big_any_policy<std::string>::print(std::ostream &out, void *const *src) {
  out << (*reinterpret_cast<std::string const *>(*src)).c_str();
}
...
public:
  ...
  bool compatible(const any &x) const { return policy->type() == x.policy->type(); }
