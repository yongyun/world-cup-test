...
class p3p
{
 public:
  ...
  p3p(Mat cameraMatrix);

  bool solve(Mat& R, Mat& tvec, const Mat& opoints, const Mat& ipoints);
 ...
 private:
  ...
  void extract_points(const Mat& opoints, const Mat& ipoints, std::vector<double>& points)
  {
      points.clear();
      points.resize(20);
      for(int i = 0; i < 4; i++)
