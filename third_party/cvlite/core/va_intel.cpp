...
#  include <va/va.h>
#else  // HAVE_VA
#  define NO_VA_SUPPORT_ERROR C8CV_ErrorNoReturn(c8cv::Error::StsBadFunc, "OpenCV was build without VA support (libva)")
#endif // HAVE_VA
...
namespace c8cv { namespace va_intel {
...
# if (defined(HAVE_VA_INTEL) && defined(HAVE_OPENCL))
    ...
    if (tryInterop)
    {
        ...
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLInitError, "OpenCL: Can't get platform Id list");
...
void convertToVASurface(VADisplay display, InputArray src, VASurfaceID surface, Size size)
{
    ...
    C8CV_Assert(srcType == stype);

    Size srcSize = src.size();
    C8CV_Assert(srcSize.width == size.width && srcSize.height == size.height);
...
# if (defined(HAVE_VA_INTEL) && defined(HAVE_OPENCL))
    if (contextInitialized)
    {
        ...
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (Y plane)");
        cl_mem clImageUV = clCreateFromVA_APIMediaSurfaceINTEL(context, CL_MEM_WRITE_ONLY, &surface, 1, &status);
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clCreateFromVA_APIMediaSurfaceINTEL failed (UV plane)");
        ...
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clEnqueueAcquireVA_APIMediaSurfacesINTEL failed");
        ...
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clEnqueueReleaseVA_APIMediaSurfacesINTEL failed");
        ...
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clReleaseMem failed (Y plane)");
        status = clReleaseMemObject(clImageUV);
        if (status != CL_SUCCESS)
            C8CV_Error(c8cv::Error::OpenCLApiCallError, "OpenCL: clReleaseMem failed (UV plane)");
    }
    else
# endif // HAVE_VA_INTEL && HAVE_OPENCL
    {
        ...
        // TODO Add support for roi
        C8CV_Assert(m.data == m.datastart);
        ...
        if (status != VA_STATUS_SUCCESS)
            C8CV_Error(c8cv::Error::StsError, "VA-API: vaSyncSurface failed");

        VAImage image;
        status = vaDeriveImage(display, surface, &image);
        if (status != VA_STATUS_SUCCESS)
            C8CV_Error(c8cv::Error::StsError, "VA-API: vaDeriveImage failed");

        unsigned char* buffer = 0;
        status = vaMapBuffer(display, image.buf, (void **)&buffer);
        if (status != VA_STATUS_SUCCESS)
            C8CV_Error(c8cv::Error::StsError, "VA-API: vaMapBuffer failed");

        C8CV_Assert(image.format.fourcc == VA_FOURCC_NV12);

        copy_convert_bgr_to_nv12(image, m, buffer);

        status = vaUnmapBuffer(display, image.buf);
        if (status != VA_STATUS_SUCCESS)
            C8CV_Error(c8cv::Error::StsError, "VA-API: vaUnmapBuffer failed");

        status = vaDestroyImage(display, image.image_id);
        if (status != VA_STATUS_SUCCESS)
            C8CV_Error(c8cv::Error::StsError, "VA-API: vaDestroyImage failed");
    }
#endif  // !HAVE_VA
}
