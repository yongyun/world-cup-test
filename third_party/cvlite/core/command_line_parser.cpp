...
static void from_str(const String& str, int type, void* dst)
{
    ...
    else if( type == Param::BOOLEAN )
    {
        ...
        *(String*)dst = str;
        ...
        C8CV_Error(Error::StsBadArg, "unknown/unsupported parameter type");

    if (ss.fail())
    {
        C8CV_Error_(Error::StsBadArg, ("can not convert: [%s] to [%s]", str.c_str(), get_type_name(type)));
    }
}
...
void CommandLineParser::getByName(const String& name, bool space_delete, int type, void* dst) const
{
    ...
    C8CV_Error_(Error::StsBadArg, ("undeclared key '%s' requested", name.c_str()));
}
...
void CommandLineParser::getByIndex(int index, bool space_delete, int type, void* dst) const
{
    ...
    C8CV_Error_(Error::StsBadArg, ("undeclared position %d requested", index));
}
...
std::vector<String> CommandLineParser::Impl::split_range_string(const String& _str, char fs, char ss) const
{
    ...
    while (!str.empty())
    {
        if (str[0] == fs)
        {
            if (begin == true)
            {
                throw c8cv::Exception(CV_StsParseError,
