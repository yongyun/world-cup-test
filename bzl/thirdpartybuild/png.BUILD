...
genrule(
    ...
    outs = ["pnglibconf.h"],
    cmd = """
echo '/* pnglibconf.h - library build configuration */' >> $@
echo '/* libpng version 1.6.37.git */' >> $@
echo '/* Copyright (c) 2018-2019 Cosmin Truta */' >> $@
echo '/* Copyright (c) 1998-2002,2004,2006-2018 Glenn Randers-Pehrson */' >> $@
echo '/* This code is released under the libpng license. */' >> $@
echo '/* For conditions of distribution and use, see the disclaimer */' >> $@
echo '/* and license in png.h */' >> $@
echo '/* pnglibconf.h */' >> $@
echo '/* Machine generated file: DO NOT EDIT */' >> $@
echo '/* Derived from: scripts/pnglibconf.dfa */' >> $@
echo '#ifndef PNGLCONF_H' >> $@
echo '#define PNGLCONF_H' >> $@
echo '/* options */' >> $@
echo '#define PNG_16BIT_SUPPORTED' >> $@
echo '#define PNG_ALIGNED_MEMORY_SUPPORTED' >> $@
echo '/*#undef PNG_ARM_NEON_API_SUPPORTED*/' >> $@
echo '/*#undef PNG_ARM_NEON_CHECK_SUPPORTED*/' >> $@
echo '#define PNG_BENIGN_ERRORS_SUPPORTED' >> $@
echo '#define PNG_BENIGN_READ_ERRORS_SUPPORTED' >> $@
echo '/*#undef PNG_BENIGN_WRITE_ERRORS_SUPPORTED*/' >> $@
...
echo '/*#undef PNG_ERROR_NUMBERS_SUPPORTED*/' >> $@
...
echo '/*#undef PNG_POWERPC_VSX_API_SUPPORTED*/' >> $@
echo '/*#undef PNG_POWERPC_VSX_CHECK_SUPPORTED*/' >> $@
...
echo '#define PNG_READ_eXIf_SUPPORTED' >> $@
...
echo '#define PNG_WRITE_eXIf_SUPPORTED' >> $@
...
echo '#define PNG_eXIf_SUPPORTED' >> $@
...
echo '#define PNG_ZLIB_VERNUM 0 /* unknown */' >> $@
...
echo '#endif /* PNGLCONF_H */' >> $@
""",
)

# Do we want Hardware Optimizations? (ARM, SSE, MIPS and PowerPC)
cc_library(
    name = "png",
    srcs = [
        "pngpriv.h",
        ...
        "pngwtran.c",
        "pngwutil.c",
        # hardware specific sources go here
    ],
    hdrs = [
        "png.h",
        "pngconf.h",
        ":pnglibconf",  # We generate this by hand instead of the AWK version
    ],
    copts = [
        "-DPNG_ARM_NEON_OPT=0",
