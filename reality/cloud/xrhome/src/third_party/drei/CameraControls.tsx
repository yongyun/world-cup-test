...
import {Box3, EventDispatcher, MathUtils, Matrix4, OrthographicCamera, PerspectiveCamera, Quaternion, Raycaster, Sphere, Spherical, Vector2, Vector3, Vector4} from 'three'
import * as React from 'react'
import {forwardRef, useMemo, useEffect} from 'react'
import {extend, useThree, useFrame, ReactThreeFiber} from '@react-three/fiber'
import CameraControlsImpl from 'camera-controls'

type CameraControlsProps = Omit<ReactThreeFiber.Overwrite<ReactThreeFiber.Node<CameraControlsImpl, typeof CameraControlsImpl>, {
...
const CameraControls = /* @__PURE__ */forwardRef((props: CameraControlsProps, ref) => {
  ...
  const controls = useMemo(() => {
    const newControls = new CameraControlsImpl(explCamera)
    if (target) {
      newControls.setTarget(target.x, target.y, target.z)
    }
    return newControls
  }, [explCamera])
