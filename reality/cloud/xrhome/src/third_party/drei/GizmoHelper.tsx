...
export const useGizmoContext = () => React.useContext<GizmoHelperContext>(Context)

const turnRate = 2 * Math.PI  // turn rate in angles per second
const dummy = /* @__PURE__ */ new Object3D()
const matrix = /* @__PURE__ */ new Matrix4()
const [q1, q2] = [new Quaternion(), /* @__PURE__ */ new Quaternion()]
const target = /* @__PURE__ */ new Vector3()
const targetPosition = /* @__PURE__ */ new Vector3()
const stepVector = /* @__PURE__ */ new Vector3()
...
const isOrbitControls = (controls: ControlsProto): controls is OrbitControlsType => 'minPolarAngle' in (controls as OrbitControlsType)
const isCameraControls = (controls: any): controls is CameraControlsImpl => 'getTarget' in (controls as CameraControlsImpl)
    ...
    (direction: Vector3) => {
      ...
      if (defaultControls || onTarget) {
        if (isCameraControls(defaultControls)) {
          ...
          focusPoint.current = defaultControls?.target || onTarget?.()
    ...
    if (virtualCam.current && gizmoRef.current) {
      ...
      if (animating.current) {
        if (q1.angleTo(q2) < 0.01) {
          ...
          // animate orientation
          if (isCameraControls(defaultControls)) {
            stepVector.set(0, 0, 1)
              .applyQuaternion(q1)
              .multiplyScalar(radius.current)
              .add(focusPoint.current)
            defaultControls.setPosition(stepVector.x, stepVector.y, stepVector.z)
          } else {
